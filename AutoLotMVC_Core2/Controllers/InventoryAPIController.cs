...
namespace AutoLotMVC_Core2.Controllers
{
    public class InventoryAPIController : Controller
    {
        ...
        public InventoryAPIController(IConfiguration configuration)
        {
        ...
        private async Task<Inventory> GetInventoryRecord(int id)
        {
            ...
            {
                Inventory inventory = JsonConvert.DeserializeObject<Inventory>(await response.Content.ReadAsStringAsync());
                return inventory;
            }
            return null;
        }
